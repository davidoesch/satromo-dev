...
def process_L57_LEVEL_2(roi,current_date):
    """
    Export the L57 Level 2 product.

    Returns:
        str: "no new imagery" if no new imagery found, None if new imagery is processed.
    """

    product_name = config.PRODUCT_L57_LEVEL_2['product_name']
    print("********* processing {} *********".format(product_name))

    # Filter the sensor collection based on date and region

    start_date = ee.Date(
        current_date).advance(-int(config.PRODUCT_L57_LEVEL_2['temporal_coverage'])+1, 'day')
    end_date = ee.Date(current_date).advance(1, 'day')

    collection = (
        ee.ImageCollection(config.PRODUCT_L57_LEVEL_2['step0_collection'])
    ...
    if main_utils.check_product_update(config.PRODUCT_L57_LEVEL_2['product_name'], sensor_stats[1]) is True:

...
3. Selects the band 'cloudProbability' for each asset, multiplies by 100, and converts it to uint8.
4. Replaces the original 'cloudProbability' band with the new uint8 band.
5. Updates metadata property 'cloud_mask_threshold' for each asset.
6. Deletes temporary assets after processing.
"""


config = r'C:\temp\topo-satromo\secrets\XXX'
...
for asset_name in asset_names:
    ...
    # Replace the 'cloudProbability' band with the new uint8 band
    image_new = image.addBands(
        image.select('cloudProbability').multiply(100).toUint8(),
        overwrite=True
    )

    # Set metadata property 'cloud_mask_threshold'
    image_new = image_new.set('cloud_mask_threshold', "40 / 20")

...
# Pre-processing pipeline for daily Landsat 8/9 surface reflectance (sr) mosaics over Switzerland
...
export100mBands = True  # options': True, 'False - defines if 100 m thermal bands are exported': 'ST_B10'
...
##############################
# VISUALISATION
vis_nfci = {'bands': ['SR_B7', 'SR_B5', 'SR_B4'], 'min': [500, 500, 500], 'max': [4000, 5000, 3000]}
...
def maskCloudsAndShadowsL89sr(image):
    ...
    isCloud = isCloud.reproject(crs=image.select('SR_B2').projection(), scale=100)
...
distinctDates_L89_sr = L89_sr.distinct('DATE_ACQUIRED').sort('DATE_ACQUIRED')
...
if swathMosaic is True:
    ...
    L89_sr = ee.ImageCollection(joinCol_L89_sr.map(mosaic_collection)).map(addMaskedPixelCount)
...
def topoCorr_SCSc_L89(img):
    ...
    bandList = ee.List(['SR_B1', 'SR_B2', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B6', 'SR_B7'])
    ...
    bandsWithoutTC = ee.List(['ST_B10', 'cloudAndCloudShadowMask', 'QA_PIXEL', 'QA_RADSAT', 'terrainShadowMask'])
...
def dataType(image):
    return image.addBands(
        image.select(['SR_B1', 'SR_B2', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B6', 'SR_B7', 'ST_B10']).round().toUint16(),
        None, True)
...
fname_30m = 'L89-sr_Mosaic_' + sensing_date_read + '_Bands-30m'  # ['SR_B1','SR_B2','SR_B3','SR_B4','SR_B5','SR_B6','SR_B7']
fname_100m = 'L89-sr_Mosaic_' + sensing_date_read + '_Band-100m'  # ['ST_B10']
fname_masks = 'L89-sr_Mosaic_' + sensing_date_read + '_Masks-30m'  # ['terrainShadowMask','cloudAndCloudShadowMask', 'TC_mask']
fname_properties = 'L89-sr_Mosaic_' + sensing_date_read + '_properties'  # ["system:time_start", "system:index", "DATE_ACQUIRED", "SPACECRAFT_ID", "WRS_PATH", "SCENE_CENTER_TIME", "SENSOR_ID", "SUN_ELEVATION", "SUN_AZIMUTH", "EARTH_SUN_DISTANCE", "COLLECTION_CATEGORY", "COLLECTION_NUMBER", "DATA_SOURCE_ELEVATION"]
fname_QAbands = 'L89-sr_Mosaic_' + sensing_date_read + '_Bands-QA'  # ['QA_PIXEL', 'QA_RADSAT']

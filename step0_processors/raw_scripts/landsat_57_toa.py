...
# Export switches
export30mBands = True  # options': True, 'False - defines if 30 m spectral bands are exported': 'B1','B2','B3','B4','B5','B7'
export60mBands = True  # options': True, 'False - defines if 100 m thermal bands are exported': 'B6_VCID_1','B6_VCID_2'
...
##############################
# TIME
end_date = ee.Date('2001-07-24')  # ee.Date(Date.now())
...
##############################
# VISUALISATION
vis_nfci = {'bands': ['B7', 'B4', 'B3'], 'min': [500, 500, 500], 'max': [4000, 5000, 3000]}
...
def maskCloudsAndShadowsL57toa(image):
    ...
    opticalBands = image.select(['B1', 'B2', 'B3', 'B4', 'B5', 'B7']).multiply(10000)
...
distinctDates_L57_toa = L57_toa.distinct('DATE_ACQUIRED').sort('DATE_ACQUIRED')
...
if swathMosaic is True:
    ...
    L57_toa = ee.ImageCollection(joinCol_L57_toa.map(mosaic_collection)).map(addMaskedPixelCount)
...
fname_30m = 'L57-toa_Mosaic_' + sensing_date_read + '_Bands-30m'  # ['B1','B2','B3','B4','B5','B7']
fname_60m = 'L57-toa_Mosaic_' + sensing_date_read + '_Band-60m'  # ['B6']
fname_masks = 'L57-toa_Mosaic_' + sensing_date_read + '_Masks-30m'  # ['terrainShadowMask','cloudAndCloudShadowMask', 'TC_mask']
fname_properties = 'L57-toa_Mosaic_' + sensing_date_read + '_properties'  # ["system:time_start", "system:index", "DATE_ACQUIRED", "SPACECRAFT_ID", "WRS_PATH", "SCENE_CENTER_TIME", "SENSOR_ID", "SUN_ELEVATION", "SUN_AZIMUTH", "EARTH_SUN_DISTANCE", "COLLECTION_CATEGORY", "COLLECTION_NUMBER", "DATA_SOURCE_ELEVATION"]
fname_QAbands = 'L57-toa_Mosaic_' + sensing_date_read + '_Bands-QA'  # ['QA_PIXEL', 'QA_RADSAT']
fname_geometries = 'L57-toa_Mosaic_' + sensing_date_read + '_Geometries'  # ['SAA', 'SZA', 'VAA', 'VZA']

...
# Pre-processing pipeline for daily Landsat 8/9 top-of-athmosphere (toa) mosaics over Switzerland


##############################
# INTRODUCTION
#
# This script provides a tool to preprocess Landsat 8/9 TOA (top-of-atmosphere) mosaics over Switzerland.
...
swathMosaic = True  # options': True, False - defines if individual scenes get mosaiced to an image swath
...
export15mBand = True  # options': True, 'False - defines if 15 m panchromatic band is exported': 'B8'
export30mBands = True  # options': True, 'False - defines if 30 m spectral bands are exported': 'B1','B2','B3','B4','B5','B6','B7'
export100mBands = True  # options': True, 'False - defines if 100 m thermal bands are exported': 'B10','B11'
exportMasks = True  # options': True, 'False - defines if masks are exported': 'terrainShadowMask','cloudAndCloudShadowMask', 'TC_mask'
exportQAbands = True  # options': True, 'False - defines if Landsat QA bands are exported': 'QA_PIXEL', 'QA_RADSAT'
...
##############################
# VISUALISATION
vis_nfci = {'bands': ['B7', 'B5', 'B4'], 'min': [500, 500, 500], 'max': [4000, 5000, 3000]}
...
L89_toa = L9_toa.merge(L8_toa)

print('L89 col size', L89_toa.size())
print('L89 toa image - first', L89_toa.first())

# Map.addLayer(L89_toa, {'bands': ['B7',  'B5',  'B4'], 'min': 0.05, 'max': [0.4, 0.5, 0.3]}, 'L89 original', False)
...
def maskCloudsAndShadowsL89toa(image):
    # DETECT CLOUDS
    ...
    opticalBands = image.select(['B.']).multiply(10000)
    thermalBands = image.select(['B10', 'B11'])

    # Find dark pixels in the image (used in the upcoming addTerrainShadow function)
    darkPixels = image.select(['B5', 'B6', 'B7']).reduce(ee.Reducer.sum()).lt(0.25) \
        .subtract(water_binary).clamp(0, 1).rename('darkPixels')

    # get the solar position
    meanAzimuth = ee.Number(image.get('SUN_AZIMUTH'))
    meanZenith = ee.Number(90).subtract(ee.Number(image.get('SUN_ELEVATION')))
    ...
    isCloud = qaMask.directionalDistanceTransform(shadowAzimuth, shadowDistance)
...
if cloudMasking is True:
    ...
    L89_toa = L89_toa.map(maskCloudsAndShadowsL89toa)
    print('L89 toa cloud masked - first', L89_toa.first())
    # Map.addLayer(L89_toa, vis_nfci, 'L89 cloud masked', False)
...
def addTerrainShadow(image):
    ...
    darkPixels = image.select(['B5', 'B6', 'B7']).reduce(ee.Reducer.sum()).lt(2500)
...
if terrainShadowDetection is True:
    ...
    L89_toa = L89_toa.map(addTerrainShadow)

    print('L89 toa terrain shadow - first', L89_toa.first())
    # Map.addLayer(L89_toa.select('terrainShadowMask'), {}, 'L89 terrain shadow mask', False)
...
distinctDates_L89_toa = L89_toa.distinct('DATE_ACQUIRED').sort('DATE_ACQUIRED')

# define the filter
filter = ee.Filter.equals(leftField='DATE_ACQUIRED', rightField='DATE_ACQUIRED')
...
joinCol_L89_toa = join.apply(distinctDates_L89_toa, L89_toa, filter)
...
if swathMosaic is True:
    ...
    L89_toa = ee.ImageCollection(joinCol_L89_toa.map(mosaic_collection)).map(addMaskedPixelCount)

    # print('L89_toa size after mosaic', L89_toa.size())
    print('L89 toa mosaiced - first', L89_toa.first())

    # display the mosaic
    imgMosaic = ee.Image(L89_toa.first())
    # Map.addLayer(imgMosaic, vis_nfci, 'L89 mosaic', False)

    # filter for data availability
    L89_toa = L89_toa.filter(ee.Filter.gte('percentData', 2))
...
def topoCorr_L89(img):
    ...
    meanZenith = ee.Number(90).subtract(ee.Number(img.get('SUN_ELEVATION')))
...
def topoCorr_SCSc_L89(img):
    ...
    bandList = ee.List(['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8'])

    # bandList = ee.List(['B2','B3','B4','B8']
    ...
    bandsWithoutTC = ee.List(
        ['B9', 'B10', 'B11', 'cloudAndCloudShadowMask', 'QA_PIXEL', 'QA_RADSAT', 'SAA', 'SZA', 'VAA', 'VZA',
         'terrainShadowMask'])
...
if topoCorrection is True:
    ...
    L89_toa = L89_toa.map(topoCorr_L89) \
        .map(topoCorr_SCSc_L89)
    # print('L89_toa size after mosaic', L89_toa.size())
    print('L89 toa topoCorrected - first', L89_toa.first())

    # Map.addLayer(L89_toa.first(), vis_nfci, 'L89 mosaic TC', False)
...
def dataType(image):
    return image.addBands(
        image.select(['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B9', 'B10', 'B11']).round().toUint16(), None,
        True)


# data type conversion
L89_toa = L89_toa.map(dataType)

# convert image collection to image (used in export)
img_exp = ee.Image(L89_toa.first())
...
fname_all = 'L89-toa_Mosaic_' + sensing_date_read + '_All'
fname_15m = 'L89-toa_Mosaic_' + sensing_date_read + '_Band-15m'  # ['B8']
fname_30m = 'L89-toa_Mosaic_' + sensing_date_read + '_Bands-30m'  # ['B1','B2','B3','B4','B5','B6','B7']
fname_100m = 'L89-toa_Mosaic_' + sensing_date_read + '_Band-100m'  # ['B10', 'B11']
fname_masks = 'L89-toa_Mosaic_' + sensing_date_read + '_Masks-30m'  # ['terrainShadowMask','cloudAndCloudShadowMask', 'TC_mask']
fname_properties = 'L89-toa_Mosaic_' + sensing_date_read + '_properties'  # ["system:time_start", "system:index", "DATE_ACQUIRED", "SPACECRAFT_ID", "WRS_PATH", "SCENE_CENTER_TIME", "SENSOR_ID", "SUN_ELEVATION", "SUN_AZIMUTH", "EARTH_SUN_DISTANCE", "COLLECTION_CATEGORY", "COLLECTION_NUMBER", "DATA_SOURCE_ELEVATION"]
fname_QAbands = 'L89-toa_Mosaic_' + sensing_date_read + '_Bands-QA'  # ['QA_PIXEL', 'QA_RADSAT']
fname_geometries = 'L89-toa_Mosaic_' + sensing_date_read + '_Geometries'  # ['SAA', 'SZA', 'VAA', 'VZA']
...
if export30mBands is True:
    ...
    task = ee.batch.Export.image.toDrive(
        image=img_exp.select(['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7']),
        scale=30,
...
# SWITCH export
if export15mBand is True:
    # Export 15 m panchromatic band
    task = ee.batch.Export.image.toDrive(
        image=img_exp.select(['B8']),
        scale=15,
        description=fname_15m,
        crs='EPSG:2056',
        region=aoi_exp,
        maxPixels=1e10,
        folder='eeExports',

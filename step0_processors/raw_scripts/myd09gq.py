...
# Pre-processing pipeline for daily MODIS Aqua surface reflectance observations
# in the red and NIR spectral bands in 250 m (MODIS product: MYD09GQ) over Switzerland


##############################
# INTRODUCTION
#
# This script provides a tool to preprocess MODIS product MYD09GQ over Switzerland.
...
exportMasks = True  # options': True, 'False - defines if masks are exported': 'terrainShadowMask','cloudAndCloudShadowMask', 'TC_mask', 'clouds_MYD09GA_state_1km'
...
# Merge qa bands from other collections to this MYD09GQ collection
MYD09GQ = MYD09GQ.linkCollection(MYD09GA, ['state_1km', 'sur_refl_b06', 'sur_refl_b07', 'SolarZenith', 'SolarAzimuth'])

# print('MYD09GQ size', MYD09GQ.size())
# Map.addLayer(MYD09GA, {'bands': ['sur_refl_b07',  'sur_refl_b02',  'sur_refl_b01'], 'min': 0, 'max': [3000, 4000, 2000]}, 'MYD09GA original', False)
# Map.addLayer(MYD09GQ, {'bands': ['sur_refl_b01',  'sur_refl_b02',  'sur_refl_b01'], 'min': 0, 'max': [3000, 4000, 3000]}, 'MYD09GQ original', False)
...
# This functions calculates the median solar position angles from the MYD09GA bands and adds them as additional properties
...
# map the function
MYD09GQ = MYD09GQ.map(setSunAngles)
...
def maskCloudsAndShadowsMOD(image):
    ...
    # extract the MYD09GA state_1km-QA_band
    ...
    image = image.updateMask(qaCloud) \
        ...
        .addBands(qaCloud.Not().rename('clouds_MYD09GA_state_1km')) \
...
if cloudMasking is True:
    ...
    MYD09GQ = MYD09GQ.map(maskCloudsAndShadowsMOD) \
        .map(addMaskedPixelCount)
    # Map.addLayer(MYD09GQ, vis_fci, 'MYD09GQ cloud masked', True)
...
if terrainShadowDetection is True:
    ...
    MYD09GQ = MYD09GQ.map(addTerrainShadow)

    # # Map.addLayer(MYD09GQ.select('terrainShadowMask'), {}, 'MYD09GQ terrain shadow mask', False)
    # Map.addLayer(MYD09GQ.select('terrainShadowFraction'), {}, 'MYD09GQ terrain shadow fraction', False)
...
def topoCorr_SCSc_MOD(img):
    ...
    bandsWithoutTC = ee.List(
        ['QC_250m', 'num_observations', 'clouds_MYD09GA_state_1km', 'cloudAndCloudShadowMask', 'terrainShadowMask',
    ...
    if cloudMasking is False:
        # remove the bands from the co-registration
        bandsWithoutTC = bandsWithoutTC.remove('clouds_MYD09GA_state_1km')
...
if topoCorrection is True:
    ...
    DEM = DEM \
        .reduceResolution(reducer=ee.Reducer.mean(), maxPixels=1024) \
        .reproject(crs=MYD09GQ.first().select('sur_refl_b01').projection())

    # apply the topographic correction function
    MYD09GQ = MYD09GQ.map(topoCorr_MOD) \
        .map(topoCorr_SCSc_MOD)
    # print('MYD09GQ size after mosaic', MYD09GQ.size())
    # Map.addLayer(MYD09GQ.first(), vis_fci, 'MYD09GQ TC', False)
...
MYD09GQ = MYD09GQ.map(dataType)

# convert image collection to an image (used in export)
img_exp = ee.Image(MYD09GQ.first())
# Map.addLayer(img_exp, vis_fci, 'MYD09GQ export', False)

# extract the image properties
img_exp_properties = ee.FeatureCollection([ee.Feature(img_exp.select([]))])

# extract the date and time
# advance to T13:45:00 for Switzerland (45degN) as the GEE MODIS Aqua time is set to T00:00:00 UTC due to its global mosaic
sensing_date = img_exp.date().advance(13.75, 'hour').format('YYYY-MM-dd_hh-mm-ss').getInfo()
sensing_date_read = sensing_date[0:10] + '_T' + sensing_date[11:19]

# define the filenames
fname_all = 'MYD09GQ_' + sensing_date_read + '_All'
fname_250m = 'MYD09GQ_' + sensing_date_read + '_Band-250m'  # ['sur_refl_b01', 'sur_refl_b02']
fname_masks = 'MYD09GQ_' + sensing_date_read + '_Masks-250m'  # ['terrainShadowMask', 'terrainShadowFraction', 'cloudAndCloudShadowMask', 'TC_mask', 'clouds_MYD09GA_state_1km']
fname_TSF = 'MYD09GQ_' + sensing_date_read + '_TSF-250m'  # ['terrainShadowFraction']
fname_QAbands = 'MYD09GQ_' + sensing_date_read + '_Bands-QA'  # ['QC_250m', 'num_observations']
fname_properties = 'MYD09GQ_' + sensing_date_read + '_properties'  # ["SolarAzimuth", "SolarZenith", "percentData", "percentMasked", "system:asset_size", "system:footprint", "system:time_start", "system:time_end", "system:index"]
...
if exportMasks is True:
    # Export masks
    task = ee.batch.Export.image.toDrive(
        image=img_exp.select(['terrainShadowMask', 'cloudAndCloudShadowMask', 'TC_mask', 'clouds_MYD09GA_state_1km']),
        scale=250,

...
# Pre-processing pipeline for daily MODIS Auqa surface reflectance observations
# in the red and NIR spectral bands in 250 m (MODIS product: MYD13Q1) over Switzerland


##############################
# INTRODUCTION
#
# This script provides a tool to preprocess MODIS product MYD13Q1 over Switzerland.
...
#
# The script is set up to export one MYD13Q1 image every 16 days.
...
start_date = end_date.advance(-16, 'day')  # this script is set up to export only one MYD13Q1 image every 16 days
...
# MYD13Q1.061
# https:#developers.google.com/earth-engine/datasets/catalog/MODIS_061_MYD13Q1
...
# Merge qa bands from other collections to this MYD13Q1 collection
MYD13Q1 = MYD13Q1.linkCollection(MOD09GA, ['SolarAzimuth'])


# print('MYD13Q1 size', MYD13Q1.size())
# Map.addLayer(MYD13Q1, {'bands': ['sur_refl_b07',  'sur_refl_b02',  'sur_refl_b01'], 'min': 0, 'max': [3000, 4000, 3000]}, 'MYD13Q1 original', True)
...
# map the functions
MYD13Q1 = MYD13Q1.map(setSunAngles) \
...
if terrainShadowDetection is True:
    ...
    MYD13Q1 = MYD13Q1.map(addTerrainShadow)

    # # Map.addLayer(MYD13Q1.select('terrainShadowMask'), {}, 'MYD13Q1 terrain shadow mask', False)
    # Map.addLayer(MYD13Q1.select('terrainShadowFraction'), {}, 'MYD13Q1 terrain shadow fraction', False)
...
def topoCorr_SCSc_MOD(img):
    ...
    bandsWithoutTC = ee.List(
        ['NDVI', 'EVI', 'DetailedQA', 'ViewZenith', 'SolarZenith', 'SolarAzimuth', 'RelativeAzimuth', 'DayOfYear',
         'SummaryQA', 'terrainShadowMask', 'terrainShadowFraction'])
...
if topoCorrection is True:
    ...
    DEM = DEM \
        .reduceResolution(reducer=ee.Reducer.mean(), maxPixels=1024) \
        .reproject(crs=MYD13Q1.first().select('sur_refl_b01').projection())

    # apply the topographic correction function
    MYD13Q1 = MYD13Q1.map(topoCorr_MOD) \
        .map(topoCorr_SCSc_MOD)
    # print('MYD13Q1 size after mosaic', MYD13Q1.size())

    # Map.addLayer(MYD13Q1.first(), vis_fci, 'MYD13Q1 TC', False)
...
def dataType(image):
    return image.addBands(
        image.select(['sur_refl_b01', 'sur_refl_b02', 'sur_refl_b03', 'sur_refl_b07']).round().toInt16(), None, True)


# data type conversion
MYD13Q1 = MYD13Q1.map(dataType)

# convert image collection to image (used in export)
img_exp = ee.Image(MYD13Q1.first())
# Map.addLayer(img_exp, vis_fci, 'MYD13Q1 export', False)
...
fname_all = 'MYD13Q1_' + sensing_date_read + '_All'
fname_VIs = 'MYD13Q1_' + sensing_date_read + '_VIs-250m'  # ['NDVI', 'EVI']
fname_250m = 'MYD13Q1_' + sensing_date_read + '_Bands-250m'  # ['sur_refl_b01','sur_refl_b02', 'sur_refl_b03', 'sur_refl_b07']
fname_masks = 'MYD13Q1_' + sensing_date_read + '_Masks-250m'  # ['terrainShadowMask', 'TC_mask']
fname_TSF = 'MYD13Q1_' + sensing_date_read + '_TSF-250m'  # ['terrainShadowFraction']
fname_QAbands = 'MYD13Q1_' + sensing_date_read + '_Bands-QA'  # ['ViewZenith', 'SolarZenith', 'SolarAzimuth', 'RelativeAzimuth'	, 'DayOfYear'	, 'SummaryQA']
fname_properties = 'MYD13Q1_' + sensing_date_read + '_properties'  # ["SolarAzimuth", "SolarZenith", "percentData", "percentMasked", "system:asset_size", "system:footprint", "system:time_start", "system:time_end", "system:index"]
...
if exportQAbands is True:
    ...
    task = ee.batch.Export.image.toDrive(
        image=img_exp.select(
            ['ViewZenith', 'SolarZenith', 'SolarAzimuth', 'RelativeAzimuth', 'DayOfYear', 'SummaryQA']),
        scale=250,

...
dailyMosaic = True                     # options': True, False - defines if individual scenes / swaths get mosaiced
...
export300mBands = True                 # options': True, 'False - defines if 250 m spectral bands are exported': 'Oa01_radiance', ... , 'Oa21_radiance'
exportMasks = True                     # options': True, 'False - defines if masks are exported': 'terrainShadowMask','cloudAndCloudShadowMask', 'TC_mask', 'clouds_QA'
...
##############################
# VISUALISATION
vis_fci = {'bands': ['Oa18_radiance',  'Oa08_radiance',  'Oa06_radiance'], 'min': 0.1, 'max': 1, 'gamma':1.2}
...
def addSunToImage(position,date):
  ...
  SolarTime = hour.multiply(60).add(minutes).add(longitude.multiply(4)).add(E)
  ...
  Phi = (lat.sin().multiply(declin.sin()).add(lat.cos().multiply(declin.cos()).multiply(w.cos()))).acos()
...
def calSunAngles(image):
  phiAndTheta = addSunToImage(aoi_CH_simplified.centroid(), ee.Date(image.get('system:time_start')))
...
def maskCloudsAndShadowsS3(image):
  ...
  darkPixels = image.select(['Oa18_radiance']).reduce(ee.Reducer.sum()).lt(0.2).subtract(water_binary).clamp(0,1)
  ...
  isCloud = isCloud.reproject(crs=image.select('Oa18_radiance').projection(), scale=1000)
...
def mosaic_collection(img):
  ...
  col = ee.ImageCollection.fromImages(img.get('date_match')).sort('system:time_start', False)
  ...
  mosaic = col.mosaic().setDefaultProjection(ee.Image(img).select('Oa01_radiance').projection()) \
                           .clip(col_geo) \
                           .copyProperties(img, ["spacecraft", "relative_orbit_num", "processing_time", "groundTrackDirection", "PRODUCT_ID", 'SolarAzimuth', 'SolarZenith'])
...
if dailyMosaic is True:
  ...
  S3_OLCI = ee.ImageCollection(joinCol_S3_OLCI.map(mosaic_collection)).map(addMaskedPixelCount)
...
def topoCorr_SCSc_MOD(img):
  ...
  bandsWithoutTC = ee.List(['quality_flags', 'clouds_QA', 'cloudAndCloudShadowMask', 'terrainShadowMask', 'terrainShadowFraction'])
...
if topoCorrection is True:
  ...
  DEM = DEM \
      ...
      # .reproject(ee.Projection("EPSG:4326").atScale(300)) \
  DEM = DEM.reproject(crs=S3_OLCI.first().select('Oa01_radiance').projection())
...
fname_300m = 'S3_OLCI_' + sensing_date_read + '_Bands-300m'          # ['Oa01_radiance', ..., 'Oa21_radiance']
fname_masks = 'S3_OLCI_' + sensing_date_read + '_Masks-300m'         # ['terrainShadowMask', 'terrainShadowFraction', 'cloudAndCloudShadowMask', 'TC_mask', 'clouds_QA']
fname_TSF = 'S3_OLCI_' + sensing_date_read + '_TSF-300m'             # ['terrainShadowFraction']
fname_QAbands = 'S3_OLCI_' + sensing_date_read + '_Bands-QA'        # ['quality_flags']
fname_properties = 'S3_OLCI_' + sensing_date_read + '_properties'   # ["SolarAzimuth", "SolarZenith", "percentData", "percentMasked", "system:asset_size", "system:footprint", "system:time_start", "system:time_end", "system:index"]
...
if exportMasks is True:
  # Export masks
  task = ee.batch.Export.image.toDrive(
    image=img_exp.select(['terrainShadowMask', 'cloudAndCloudShadowMask', 'TC_mask', 'clouds_QA']),
    scale=300,
